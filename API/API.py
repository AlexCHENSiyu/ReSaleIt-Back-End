...
def mongodb_init():
    # connect to mongodb
    mongo = pymongo.MongoClient(host='16.162.42.168', port=27017, username="root", password="1647#4hkust",
                                authSource='admin')
...
def GetPosts():
    ...
    sorted_Posts = sorted(Posts, key=itemgetter('Score'), reverse=True) # 降序排序
    return_data['Posts'] = sorted_Posts
    return_data["Success"] = True
